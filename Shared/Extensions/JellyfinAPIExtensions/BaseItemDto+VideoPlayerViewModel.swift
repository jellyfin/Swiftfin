...
extension BaseItemDto {
    ...
    // MARK: Response to ViewModels
    
    func createVideoPlayerViewModel(from response: PlaybackInfoResponse) -> [VideoPlayerViewModel] {
	...
	func createVideoPlayerViewModel() -> AnyPublisher<[VideoPlayerViewModel], Error> {
		...
		return MediaInfoAPI.getPostedPlaybackInfo(itemId: self.id!,
			...
			.map { response -> [VideoPlayerViewModel] in
                return self.createVideoPlayerViewModel(from: response)
			}
			.eraseToAnyPublisher()
	}
