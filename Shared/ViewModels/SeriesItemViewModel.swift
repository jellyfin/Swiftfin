...
final class SeriesItemViewModel: ItemViewModel {
	...
	private func getNextUp() {
		...
		LogManager.shared.log.debug("Getting next up for show \(item.id!) (\(item.name!))")
		TvShowsAPI.getNextUp(userId: SessionManager.main.currentLogin.user.id,
		                     fields: [.primaryImageAspectRatio, .seriesPrimaryImage, .seasonUserData, .overview, .genres, .people],
		                     seriesId: item.id!, enableUserData: true)
	...
	private func requestSeasons() {
		LogManager.shared.log.debug("Getting seasons of show \(item.id!) (\(item.name!))")
