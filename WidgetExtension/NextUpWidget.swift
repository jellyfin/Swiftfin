...
struct NextUpWidgetProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> Void) {
        ...
        .sink(receiveCompletion: { result in
            ...
            dispatchGroup.notify(queue: .main) {
                completion(Timeline(
                    entries: [NextUpEntry(date: currentDate, items: downloadedItems, error: nil)],
        ...
        return Link(destination: url, label: {
            VStack(alignment: .leading) {
                ...
                Text(item.0.seriesName ?? "")
                    ...
                    .lineLimit(1)
                Text(
                    "\(item.0.name ?? "") · \(L10n.seasonAndEpisode(String(item.0.parentIndexNumber ?? 0), String(item.0.indexNumber ?? 0)))"
                )
                .font(.caption)
                .fontWeight(.semibold)
                            ...
                            ZStack(alignment: .bottomLeading) {
                                ...
                                VStack(alignment: .leading, spacing: 8) {
                                    Text(firstItem.0.seriesName ?? "")
                                        ...
                                        .frame(minWidth: 0, maxWidth: .infinity, alignment: .leading)
                                    Text(
                                        "\(firstItem.0.name ?? "") · \(L10n.seasonAndEpisode(String(firstItem.0.parentIndexNumber ?? 0), String(firstItem.0.indexNumber ?? 0)))"
                                    )
                                    .font(.caption)
                                    .fontWeight(.semibold)
...
private struct InnerShadow: ViewModifier {
    ...
    func body(content: Content) -> some View {
        GeometryReader { geo in
            content
                .overlay(
                    LinearGradient(gradient: Gradient(colors: self.colors), startPoint: .top, endPoint: .bottom)
                        .frame(height: self.radius * self.minSide(geo)),
                    alignment: .top
                )
                .overlay(
                    LinearGradient(gradient: Gradient(colors: self.colors), startPoint: .bottom, endPoint: .top)
                        .frame(height: self.radius * self.minSide(geo)),
                    alignment: .bottom
                )
        }
