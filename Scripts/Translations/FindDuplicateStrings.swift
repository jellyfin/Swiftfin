...
// `--dry` previews replacements, `--replace` applies them
...
// Rank a duplicate group so the shortest key is first (alphabetical tie-break)
...
// Map each duplicate key to its canonical (shortest) sibling
...
// Default: report and exit with error
if !isDryRun, !isReplace {
    print(
...
// Rewrite `L10n.<old>` → `L10n.<new>` in a Swift file. The `\b` word boundary
// prevents `L10n.foo` from clobbering `L10n.fooBar` when `foo` is remapped.
...
func scanDirectory(_ path: String) {
    ...
    for case let file as String in enumerator {
        ...
        // Skip the SwiftGen output; it gets regenerated after the run
