...
extension ItemView {
    ...
    struct HeaderContentGroup: ContentGroup {
        ...
        private let provider: ItemContentGroupProvider

        init(provider: ItemContentGroupProvider) {
            self.id = "\(provider.id)-header"
...
//                            )
    ...
    struct RegularEnhancedScrollView<Content: View>: ScrollContainerView {
        ...
        private var backgroundImageURL: URL? {
            backgroundImageSources.first?.url
        }
        ...
        private var background: some View {
            ...
            .animation(.linear(duration: 0.1), value: backgroundImageURL)
            .accessibilityHidden(true)
        }

        @ViewBuilder
        private var scrollContentBackground: some View {
            BlurView(style: .dark)
                .mask {
                    VStack(spacing: 0) {
                        CinematicContentGroupContainer {
