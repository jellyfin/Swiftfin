            ...
            GeometryReader { proxy in
                ZStack {
                    ...
                    ScrollView(.vertical, showsIndicators: false) {
                        VStack(spacing: 0) {
                            CinematicHeaderView(provider: provider)
                                .if(provider.item.type == .series) { view in
                                    view
                                        .focusGuide(
                                            focusGuide,
                                            tag: "header",
            ...
            VStack(alignment: .leading) {
                ...
                HStack(alignment: .bottom) {
                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        OverviewView(item: provider.item)
                            .taglineLineLimit(1)
                            .overviewLineLimit(3)
                    ...
                    VStack(spacing: 30) {
                        if provider.item.type == .person || provider.item.type == .musicArtist {
                            ImageView(provider.item.imageSource(.primary, environment: ImageSourceOptions(maxWidth: 450)))
                                .failure {
