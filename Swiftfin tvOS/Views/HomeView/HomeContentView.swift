...
extension HomeView {
    struct ContentView: View {
        ...
        @FocusState
        private var focusedImage: String?

        @State
        private var enlargedItem: BaseItemDto?
        @State
        private var hasHero = true
        @State
        private var heroVisible = true

        @State
        var recentlyAddedViewModel: ItemTypeLibraryViewModel?
        @State
        var recentlyAddedLibrariesViewModels: [String: LibraryViewModel] = [:]
        var topRatedMoviesViewModel = ItemTypeLibraryViewModel(
            itemTypes: [.movie],
            filters: .init(sortOrder: [APISortOrder.descending.filter], sortBy: [SortBy.communityRating.filter])
        )
            ...
            GeometryReader { geoReader in
                ScrollView(.vertical) {
                    ScrollViewReader { scrollView in
                        ...
                        // Hero section
                        if hasHero {
                            // Substract the hight of the first column plus the hight of its text (20) and a bit of spacing from the hight
                            // of the screen. Because all tvOS devices are 16:9 and use the same amount of points at any resolution this is
                            // fine
                            Spacer(
                ...
                .background {
                    ZStack(alignment: .bottom) {
                        if heroVisible, let enlargedItem = enlargedItem {
                            ...
                            // The alternative would be to make ImageView react to changes (i can't be bothered) or to use AsyncImage but i
                            // think it is prefered to use the custom ImageView
                ...
                .onAppear {
                    ...
                    if recentlyAddedLibrariesViewModels.isEmpty {
                        viewModel.libraries.forEach { library in
                            recentlyAddedLibrariesViewModels[library.id!] = LibraryViewModel(
                                parent: library,
                                type: .library,
                                filters: .recent
                            )
                ...
                .environment(\.hasHero, hasHero)
                .environment(\.heroVisible, heroVisible)
            }
        }
...
private struct HasHeroKey: EnvironmentKey {
    static let defaultValue: Bool = false
}

private struct HeroVisibleKey: EnvironmentKey {
    static let defaultValue: Bool = false
}
...
extension EnvironmentValues {
    ...
    var hasHero: Bool {
        get { self[HasHeroKey.self] }
        set { self[HasHeroKey.self] = newValue }
    }

    var heroVisible: Bool {
        get { self[HeroVisibleKey.self] }
        set { self[HeroVisibleKey.self] = newValue }
    }
}
