...
struct CinematicItemSelector<Item: Poster>: View {
    ...
    private func updateSelection(index: Int, resetsProgress: Bool) {
        ...
        selectionChanged(heroItems[safe: nextIndex])
    }
}
...
extension CinematicItemSelector {
    ...
    func sectionTitle(_ title: String) -> Self {
        copy(modifying: \.sectionTitle, with: title)
    }
    ...
    func onSelectionChanged(_ action: @escaping (Item?) -> Void) -> Self {
        copy(modifying: \.selectionChanged, with: action)
    }
}
