...
class VideoPlayerSettingsView: UINavigationController {
	...
	override func viewDidLoad() {
		...
		viewControllers = [UIHostingController(rootView: VideoPlayerSettings(delegate: playerDelegate ?? PlayerViewController()))]
	}

	override func viewWillDisappear(_ animated: Bool) {
		super.viewWillDisappear(animated)
		playerDelegate?.settingsPopoverDismissed()
	}
}
...
struct VideoPlayerSettings: View {
	...
	@State
	var captionTrack: Int32 = -99
	@State
	var audioTrack: Int32 = -99
	@State
	var playbackSpeedSelection: Int = 3
