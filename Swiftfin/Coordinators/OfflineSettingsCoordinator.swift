...
final class OfflineSettingsCoordinator: NavigationCoordinatable {

    let stack = NavigationStack(initial: \OfflineSettingsCoordinator.start)
    ...
    func makeStart() -> some View {
        let viewModel = SettingsViewModel(server: SessionManager.main.currentLogin.server,
                                          user: SessionManager.main.currentLogin.user)
        OfflineSettingsView(viewModel: viewModel)
    }
}
