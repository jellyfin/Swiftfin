...
extension ItemView {
    ...
    struct CinematicScrollView<Content: View>: ScrollContainerView {
        ...
        private var headerView: some View {
            ...
            .onChange(of: provider.item.imageSource(imageType, environment: ImageSourceOptions()).url) { _ in
                bottomColor = nil
            }
        }
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 10) {
                VStack(alignment: .center, spacing: 10) {
                    if !usePrimaryImage {
                        ImageView(provider.item.imageSource(.logo, environment: ImageSourceOptions(maxHeight: 100)))
                            .placeholder { _ in
                                EmptyView()
                            }
                            .failure {
