...
class VideoPlayerViewController: UIViewController, VideoPlayerSettingsDelegate, VLCMediaPlayerDelegate, VLCMediaDelegate,
UIGestureRecognizerDelegate {

	@IBOutlet
	weak var videoContentView: UIView!
	@IBOutlet
	...
	@IBOutlet
	weak var transportBarView: UIView!
	@IBOutlet
	weak var scrubberView: UIView!
	@IBOutlet
	weak var scrubLabel: UILabel!
	@IBOutlet
	...
	@IBOutlet
	weak var infoPanelContainerView: UIView!
	...
	func fetchVideo() {
		...
		let playbackInfo = PlaybackInfoDto(userId: currentUser.id, maxStreamingBitrate: Int(maxBitrate),
		                                   startTimeTicks: manifest.userData?.playbackPositionTicks ?? 0, deviceProfile: profile,
		                                   autoOpenLiveStream: true)

		DispatchQueue.global(qos: .userInitiated).async { [self] in
			MediaInfoAPI.getPostedPlaybackInfo(itemId: manifest.id!, userId: currentUser.id, maxStreamingBitrate: Int(maxBitrate),
				...
				.sink(receiveCompletion: { result in
					...
					// Item will be directly played by the client
						...
						streamURL =
							URL(string: "\(SessionManager.main.currentLogin.server.uri)/Videos/\(manifest.id!)/stream?Static=true&mediaSourceId=\(manifest.id!)&Tag=\(mediaSource.eTag ?? "")")!
					}
					...
					for stream in mediaSource.mediaStreams! {
						...
						if stream.type == .audio {
							let track = AudioTrack(name: stream.displayTitle!, languageCode: stream.language ?? "",
	...
	func pause() {
		...
		updateNowPlayingCenter(time: nil, playing: false)

		animateScrubber()

		scrubLabel.frame = CGRect(x: scrubberView.frame.minX - scrubLabel.frame.width / 2, y: scrubLabel.frame.minY,
		                          width: scrubLabel.frame.width, height: scrubLabel.frame.height)
	}
	...
	func toggleInfoContainer() {
		...
		if showingInfoPanel, seeking {
			...
			UIView.animate(withDuration: 0.4, delay: 0, options: .curveEaseOut, animations: {
				self.scrubberView.frame = CGRect(x: self.initialSeekPos, y: self.scrubberView.frame.minY, width: 2,
				                                 height: self.scrubberView.frame.height)
			}) { _ in
				self.scrubLabel.frame = CGRect(x: self.initialSeekPos - self.scrubLabel.frame.width / 2, y: self.scrubLabel.frame.minY,
	...
	func setupGestures() {
		...
		let playPauseGesture = UITapGestureRecognizer(target: self, action: #selector(selectButtonTapped))
		...
		let backTapGesture = UITapGestureRecognizer(target: self, action: #selector(backButtonPressed(tap:)))
	...
	@objc
	func backButtonPressed(tap: UITapGestureRecognizer) {
		// Dismiss info panel
		if showingInfoPanel {
			if focusedOnTabBar {
				toggleInfoContainer()
	...
	func userPanned(panGestureRecognizer: UIPanGestureRecognizer) {
		...
		if !seeking {
			initialSeekPos = scrubberView.frame.minX
			seeking = true
			scrubLabel.isHidden = false
		}

		let newPos = (scrubberView.frame.minX + velocity.x / 100).clamped(to: 0 ... transportBarView.frame.width)
	...
	func selectButtonTapped() {
		...
		if seeking {
			...
			UIView.animate(withDuration: 0.4, delay: 0, options: .curveEaseOut, animations: { [self] in
				...
				self.currentTimeLabel.frame = CGRect(x: CGFloat(scrubLabel.frame.minX + transportBarView.frame.minX),
				                                     y: currentTimeLabel.frame.minY, width: currentTimeLabel.frame.width,
				                                     height: currentTimeLabel.frame.height)

			})

			let time = (Double(scrubberView.frame.minX) * videoDuration) / Double(transportBarView.frame.width)

			currentTimeLabel.text = scrubLabel.text
			remainingTimeLabel.text = "-" + formatSecondsToHMS(videoDuration - time)

			mediaPlayer.position = Float(scrubberView.frame.minX) / Float(transportBarView.frame.width)
	...
	func sendProgressReport(eventName: String) {
		...
		if (eventName == "timeupdate" && mediaPlayer.state == .playing) || eventName != "timeupdate" {
			...
			let progressInfo = PlaybackProgressInfo(canSeek: true, item: manifest, itemId: manifest.id, sessionId: playSessionId,
			                                        ...
			                                        subtitleStreamIndex: Int(selectedCaptionTrack), isPaused: !playing, isMuted: false,
			                                        positionTicks: ticks, playbackStartTimeTicks: Int64(startTime), volumeLevel: 100,
			                                        brightness: 100, aspectRatio: nil, playMethod: playbackItem.videoType,
	...
	func mediaPlayerTimeChanged(_ aNotification: Notification!) {
		...
		if time != lastTime {
			currentTimeLabel.text = formatSecondsToHMS(Double(mediaPlayer.time.intValue / 1000))
			remainingTimeLabel.text = "-" + formatSecondsToHMS(Double(abs(mediaPlayer.remainingTime.intValue / 1000)))

			videoPos = Double(mediaPlayer.position)

			let newPos = videoPos * Double(transportBarView.frame.width)
			if !newPos.isNaN, playing {
				scrubberView.frame = CGRect(x: newPos, y: 0, width: 2, height: 10)
				currentTimeLabel.frame = CGRect(x: CGFloat(newPos) + transportBarView.frame.minX - currentTimeLabel.frame.width / 2,
				                                y: currentTimeLabel.frame.minY, width: currentTimeLabel.frame.width,
				                                height: currentTimeLabel.frame.height)
			}
	...
	func setupInfoPanel() {
		infoTabBarViewController?.setupInfoViews(mediaItem: manifest, subtitleTracks: subtitleTrackArray,
		                                         selectedSubtitleTrack: selectedCaptionTrack, audioTracks: audioTrackArray,
		                                         selectedAudioTrack: selectedAudioTrack, delegate: self)
	}
	...
	func formatSecondsToHMS(_ seconds: Double) -> String {
		...
		guard !seconds.isNaN,
		      let text = timeHMSFormatter.string(from: seconds)
		else {
			return "00:00"
		}
