...
UIGestureRecognizerDelegate {
	...
	func pause() {
		...
		self.scrubLabel.frame = CGRect(x: self.scrubberView.frame.minX - self.scrubLabel.frame.width / 2, y: self.scrubLabel.frame.minY,
	...
	func mediaPlayerTimeChanged(_ aNotification: Notification!) {
		...
		if time != lastTime {
			...
			if !newPos.isNaN && self.playing {
				...
				self.currentTimeLabel.frame = CGRect(x: CGFloat(newPos) + transportBarView.frame.minX - currentTimeLabel.frame.width / 2,
				                                     y: currentTimeLabel.frame.minY, width: currentTimeLabel.frame.width,
				                                     height: currentTimeLabel.frame.height)
			}
